...
cd droidian
cp -r * /mnt/
chroot /mnt/ /bin/bash -c 'export PATH="$PATH:/usr/bin:/usr/sbin" && systemctl enable samsung-hwc && systemctl enable apt-fix && chmod 755 /var/lib/lxc/android/pr* && dpkg -i /tmp/debs/*.deb && rm -rf /tmp/debs'
umount /mnt/
echo "Done!"
